...
export const MembersModal: FC = () => {
  ...
  const onKick = useCallback((memberId: string) => async () => {
    try {
  ...
  const onRoleChange = useCallback((memberId: string, role: MemberRole) => async () => {
    try {
      setLoadingId(memberId);
      ...
      const response = await axios.patch<Server>(url, { role });
